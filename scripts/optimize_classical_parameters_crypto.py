...
def backtest_wrapper(params, raw_data, initial_capital=10000, transaction_fee=0.0025):
    """
...
def optimize_parameters_parallel(raw_data, param_grid, initial_capital=10000,
                                 transaction_fee=0.0025, max_workers=None):
    """
...
def optimize_one(ticker: str, grid: dict, odir="./"):
    raw_data = load_ccxt_data(ticker, exchange_name="binance", interval="1d",
                            start_date="2000-01-02", end_date="2023-01-01", limit=None)
    ...
    results_df.to_csv(f"{odir}/optimization_results_{ticker.replace("/", "_")}.csv", index=False)
    # Tester les meilleurs paramètres sur une période de validation différente
    validation_data = load_ccxt_data(ticker, exchange_name="binance", interval="1d",
                   start_date="2023-01-01", end_date="2025-09-01", limit=None)
...
if __name__ == "__main__":
    ...
    odir = f"{config_path}/classical_strategy_crypto/"
